...
namespace Eman.Application.Modules.Engineering.Bom.Mau.BomMauHeSoDeTai.Services;

public sealed class BomMauHeSoDeTaiService(
    IBomMauHeSoDeTaiRepository repository,
    IHeSanPhamRepository heSanPhamRepository,
    IDeTaiRepository deTaiRepository,
    IBomMauBuocRepository buocRepository,
    IUnitOfWork unitOfWork) : IBomMauHeSoDeTaiService
{
    public async Task<PagedResult<BomMauHeSoDeTaiDto>> LayDanhSachAsync(BoLocBomMauHeSoDeTaiRequest request, CancellationToken cancellationToken)
    {
        var (items, totalCount) = await repository.LayDanhSachAsync(request, cancellationToken);
        return new PagedResult<BomMauHeSoDeTaiDto> { Items = items.Select(ChuyenDto).ToList(), Page = request.Page, PageSize = request.PageSize, TotalCount = totalCount };
    }

    public async Task<BomMauHeSoDeTaiDto> LayTheoIdAsync(long id, CancellationToken cancellationToken)
        => ChuyenDto(await repository.LayTheoIdAsync(id, false, cancellationToken)
            ?? throw new KhongTimThayException("Không tìm thấy hệ số đề tài."));

    public async Task<BomMauHeSoDeTaiDto> TaoMoiAsync(TaoBomMauHeSoDeTaiRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await repository.TonTaiTrungAsync(request.HeSanPhamId, request.DeTaiId, request.BuocId, null, cancellationToken))
            throw new XungDotDuLieuException("Hệ số đề tài đã tồn tại với cùng thông tin khóa.");
    ...
    public async Task<BomMauHeSoDeTaiDto> CapNhatAsync(long id, CapNhatBomMauHeSoDeTaiRequest request, CancellationToken cancellationToken)
    {
        ...
        var heSo = request.HeSo;
        var ghiChu = ChuoiHelper.ChuanHoaTuyChon(request.GhiChu);
        if (await repository.TonTaiTrungAsync(request.HeSanPhamId, request.DeTaiId, request.BuocId, id, cancellationToken))
            throw new XungDotDuLieuException("Hệ số đề tài đã tồn tại với cùng thông tin khóa.");
        ...
        entity.TenDeTai = deTai.TenDeTai;
    ...
    private static BomMauHeSoDeTaiDto ChuyenDto(Entity entity)
        => new()
        {
            Id = entity.Id,
