...
namespace Eman.Infrastructure.Persistence.Configurations.Engineering.Bom.Mau.BomMauHeSoMau;

public sealed class BomMauHeSoMauConfiguration : IEntityTypeConfiguration<Entity>
{
    public void Configure(EntityTypeBuilder<Entity> builder)
    {
        builder.ToTable("md_bom_mau_he_so_mau", "dbo", table => table.HasCheckConstraint("CK_md_bom_mau_he_so_mau_he_so", "[he_so] >= 0"));
        ...
        builder.Property(x => x.MauSacId).HasColumnName("mau_sac_id").IsRequired();
        ...
        builder.HasIndex(x => new { x.HeSanPhamId, x.DeTaiId, x.MauSacId, x.BuocId }).IsUnique().HasDatabaseName("UQ_md_bom_mau_he_so_mau");
        ...
        builder.HasOne(x => x.MauSac).WithMany().HasForeignKey(x => new { x.HeSanPhamId, x.DeTaiId, x.MauSacId }).HasPrincipalKey(x => new { x.HeSanPhamId, x.DeTaiId, x.Id }).OnDelete(DeleteBehavior.Restrict);
